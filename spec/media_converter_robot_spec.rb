...
module CF
  describe CF::MediaConverterRobot do
    context "create a google translator worker" do
      ...
      xit "should create google_translate_robot worker for multiple station in a plain ruby way" do
        # WebMock.allow_net_connect!
        VCR.use_cassette "media_converter_robot/block/create-worker-multiple-station", :record => :new_episodes do
          line = CF::Line.create("Media Converter Robot","Digitization") do |l|
          ...
          station = CF::Station.new({:type => "work", :input_formats => {:except => ["Website","Company"]}})
          line.stations station
          ...
          run = CF::Run.create(line, "media converter robot", [{"Company" => "Apple", "Website" => "www.apple.com", "url"=> "www.s3.amazon.com/converted/20110518165230/ten.mpg", "to" => "mpg", "audio_quality" => "320", "video_quality" => "3",  "meta_data_text"=>"media"}])
