...
module CF
  describe CF::TermExtractionRobot do
    context "create a sentiment robot worker" do
      it "should create content_scraping_robot worker for first station in Block DSL way" do
        ...
        VCR.use_cassette "term_extraction_robot/block/create-worker-single-station", :record => :new_episodes do
          line = CF::Line.create("term_extraction_42","Digitization") do |l|
            CF::InputFormat.new({:line => l, :name => "url", :valid_type => "url", :required => "true"})
            CF::Station.create({:line => l, :type => "work"}) do |s|
              CF::TermExtractionRobot.create({:station => s, :url => ["{url}"], :max_retrieve => 5, :show_source_text => true})
            end
          end
          run = CF::Run.create(line, "term_extraction_run_42", [{"url"=> "http://www.sprout-technology.com"}])
          ...
          line.stations.first.worker.class.should eql(CF::TermExtractionRobot)
      ...
      it "should create content_scraping_robot worker for first station in a plain ruby way" do
        # WebMock.allow_net_connect!
        VCR.use_cassette "term_extraction_robot/plain/create-worker-in-first-station", :record => :new_episodes do
          line = CF::Line.new("term_extraction_72","Digitization")
          ...
          worker = CF::TermExtractionRobot.create({:url => ["{url}"], :max_retrieve => 5, :show_source_text => true})
          line.stations.first.worker = worker

          run = CF::Run.create(line, "term_extraction_run_72", [{"url"=> "http://www.sprout-technology.com"}])
          output = run.final_output
          output.first.final_output.first.keyword_relevance_of_url.should eql([99.7991, 95.6566, 83.2383, 79.39450000000001, 76.0281])
          output.first.final_output.first.keywords_of_url.should eql(["Web App", "web application", "Web App Development", "Web App Management", "world-class web development"])
          output.first.final_output.first.max_retrieve.should eql("5")
