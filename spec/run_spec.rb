  ...
  describe CloudFactory::Run do
    context "create a new run" do
      it "the plain ruby way" do
        ...
        input_header1 = CloudFactory::InputHeader.new({:label => "Name",:field_type => "text_data", :required => true})
        input_header2 = CloudFactory::InputHeader.new({:label => "Age",:field_type => "text_data", :required => true})

        instruction =   CloudFactory::StandardInstruction.create(:title => "Title", :description => "Describe") do |i|
          i.form_fields = form_fields
        end
        line = CloudFactory::Line.create("Google Translator") do |l|
