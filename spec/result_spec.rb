    ...
    context "Trace result" do
      it "should trace the result of a production run" do
        VCR.use_cassette "result/block/create-run", :record => :new_episodes do
          line = CF::Line.create("CEO","Survey") do |l|
            CF::Station.create({:line => l, :type => "work"}) do |s|
              ...
              CF::TaskForm.create({:station => s, :title => "Enter the names of CEO", :description => "Enter the full name of CEO of the following company"}) do |i|
          ...
          got_result[1].meta_data['company'].should eql("Gmail")
          got_result[1].results.last['first-name'].should eql("Larry")
          got_result[1].results.last['last-name'].should eql("Paige")
          got_result[0].meta_data['company'].should eql("Facebook")
          got_result[0].results.last['first-name'].should eql("Mark")
          got_result[0].results.last['last-name'].should eql("Zuckerberg")
          got_result[2].meta_data['company'].should eql("Apple")
          got_result[2].results.last['first-name'].should eql("Bob")
          got_result[2].results.last['last-name'].should eql("Marley")
        end
      end
    end
  end
